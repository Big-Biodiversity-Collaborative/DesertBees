...
# Maxent for Olneya tesota

# Current and future predictions of O. tesota distribution
...
species_SDM <- dismo::maxent(x = pres_abs_env,
                        p = pres_abs,
                        path = paste("output/maxent_Otesota_outputs"), )
...
title <- ggdraw() + 
  draw_label(bquote("Species Distribution Model of" ~italic("Olneya tesota")),
             fontface = "bold",
             hjust = 0.5,
             size = 20)
...
# Save combined plots
ggsave(filename = "current_future_SDM_otesota.jpg",
       plot = combined_plots,
