...
# Reason:
  # To get bioclimatic variables for 2021-2040, 2041-2060, 2061-2080, and 2081-2100
  # Forecast data for 20-year periods, rather than 30-year periods
...
# Calculates average values for the 19 bioclimatic variables for four time spans, 
  # 2021-2040, 2041-2060, 2061-2080, and 2081-2100,
  # based on monthly values for the 20 year span. 

# The monthly data are based on ensemble forecasts under SSP 7.0
...
# There are multiple bouts of iteration over the same combinations of variables, 
  # while this processing could probably be done with a single bout of 
...
# Set to 30 minutes, files are large (1.3GB)
...
for (ssp in ssps) {
  ...
  for (per_start in per_starts) {
    ...
    per_end <- as.integer(per_start) + 19
    time_per <- paste0(per_start, "_", per_end)

    # Download zip file if it doesn't exist
    zip_file <- paste0("data/ensemble/monthly/ensemble_", 
                       ssp, "_",
...
template_raster <- terra::rast(x = "data/worldclim/bio1.tif")
...
for (ssp in ssps) {
  ...
  for (per_start in per_starts) {
    ...
    if (replace_rasters | !files_exist) {
      ...
      # End conditional for file exists or force replacement
...
# Remove initial download folders now that they've been coverted to monthly
  # bioclimatic values

# There IS a part for this in the first loop, but it seems there's an issue
  # and an error of "Permission denied" in the attempt to remove the empty
  # directories
...
for (ssp in ssps) {
  ...
  for (per_start in per_starts) {
    ...
    # Get folder path
    folder_name <- paste0("data/ensemble/",
                          "ensemble_8GCMs_", ssp, "_", time_per)
